    ...
    <div>
          ...
          >
            <div className="rounded-2xl border border-primary/20 bg-card p-5 space-y-4 shadow-sm">
              ...
              <div>
                ...
                <Input placeholder="e.g. Bleacher Boys" value={newName} onChange={e => setNewName(e.target.value)} maxLength={40} className="rounded-xl" />
              </div>
              <div>
                <Label className="text-xs text-muted-foreground mb-1 block">Description</Label>
                <Input placeholder="What's your crew about?" value={newDesc} onChange={e => setNewDesc(e.target.value)} maxLength={120} className="rounded-xl" />
              </div>
        ...
        <div className="mb-6">
          ...
          <div className="space-y-2">
            {myCrews.map((crew, i) => (
              ...
              >
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 text-2xl">{crew.badge_emoji}</div>
      ...
      <div>
        ...
        <div className="relative mb-3">
          ...
          <Input placeholder="Search crews..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9 rounded-xl" />
        </div>
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {filtered.map((crew, i) => (
              ...
              >
                <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-accent/10 text-2xl">{crew.badge_emoji}</div>
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-foreground truncate">{crew.name}</p>
                  {crew.description && <p className="text-xs text-muted-foreground truncate">{crew.description}</p>}
