  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-start gap-3">
        ...
        >
          <ConceptVisual name={moment.emoji} size="sm" />
        </motion.div>
...
export function LiveMomentsPanel() {
  ...
  const handleCreate = (preset: typeof MOMENT_PRESETS[number]) => {
    createMoment.mutate(
      ...
      {
        onSuccess: () => {
          toast.success(`$<ConceptVisual name={preset.emoji} size="sm" /> "${preset.title}" is live!`);
          setShowCreate(false);
        },
      }
    );
                ...
                {showCreate ? (
                  ...
                  >
                    {MOMENT_PRESETS.map((preset) => (
                      ...
                      >
                        <span className="text-lg"><ConceptVisual name={preset.emoji} size="sm" /></span>
