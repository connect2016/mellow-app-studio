...
export default function MemoriesContent() {
  ...
  const { data: dayStats } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { count: matchCount } = await supabase.from('matches').select('id', { count: 'exact', head: true }).or(`user_a.eq.${user.id},user_b.eq.${user.id}`).gte('created_at', todayISO);
      const { count: hiFiveCount } = await supabase.from('likes').select('id', { count: 'exact', head: true }).eq('from_user', user.id).eq('is_hi_five', true).gte('created_at', todayISO);
      const { count: memoryCount } = await supabase.from('game_memories').select('id', { count: 'exact', head: true }).eq('user_id', user.id).gte('created_at', todayISO);
      return { newFans: matchCount ?? 0, hiFives: hiFiveCount ?? 0, memories: memoryCount ?? 0 };
    },
    enabled: !!user,
  });
  ...
  const uploadMemory = useMutation({
    mutationFn: async () => {
      ...
      const { error: uploadErr } = await supabase.storage.from('vibe-media').upload(path, selectedFile, { upsert: true });
  ...
  return (
    <div>
      ...
      {dayStats && (dayStats.newFans > 0 || dayStats.hiFives > 0 || dayStats.memories > 0) && (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mb-4 rounded-2xl border border-border bg-card p-4 relative overflow-hidden">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }} className="mb-4 rounded-2xl border border-border bg-card overflow-hidden shadow-lg">
            ...
            <div className="p-4 space-y-3">
              ...
              <div>
                ...
                <input type="text" placeholder="Search fans to tag..." value={tagSearch} onChange={e => setTagSearch(e.target.value)} className="w-full rounded-xl border border-border bg-muted/50 px-3 py-2 text-sm placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/30" />
                {searchResults.length > 0 && tagSearch.length >= 2 && (
                  <div className="mt-1.5 rounded-xl border border-border bg-card overflow-hidden">
                    {searchResults.map(sr => (
                      <button key={sr.user_id} onClick={() => { if (!taggedUsers.find(t => t.id === sr.user_id)) { setTaggedUsers(prev => [...prev, { id: sr.user_id, name: sr.display_name, photo: sr.profile_photo || '' }]); } setTagSearch(''); }} className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-muted/50 transition-colors">
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {memories.map((memory, idx) => {
            ...
            const tagged = ((memory.tagged_users as string[]) || []).map(id => profileMap[id]).filter(Boolean);
            const isOwn = memory.user_id === user?.id;
            return (
              <motion.div key={memory.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.05 }} className="rounded-2xl border border-border bg-card overflow-hidden shadow-sm">
                <div className="flex items-center gap-2.5 px-3 py-2.5">
                  <Avatar className="h-8 w-8 ring-2 ring-primary/20">
                    <AvatarImage src={author?.profile_photo || ''} />
                    <AvatarFallback className="bg-primary/10 text-primary text-xs font-bold">{(author?.display_name || '?')[0]}</AvatarFallback>
                  </Avatar>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-foreground truncate">{author?.display_name || 'Fan'}</p>
                    <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                      <span className="flex items-center gap-0.5"><MapPin className="h-3 w-3" />{memory.location_tag}</span>
                      <span>·</span>
                      <span>{timeAgo(memory.created_at)}</span>
                    </div>
                  </div>
                  {isOwn && (
                    <button onClick={() => deleteMemory.mutate(memory.id)} className="p-1.5 rounded-full hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors">
                      <Trash2 className="h-4 w-4" />
                    </button>
                  )}
                </div>
                <div className="aspect-[4/3] bg-muted">
                  <img src={memory.media_url} alt={memory.caption || 'Memory'} className="h-full w-full object-cover" loading="lazy" />
                </div>
                {(memory.caption || tagged.length > 0) && (
                  <div className="px-3 py-2.5">
                    {memory.caption && <p className="text-sm text-foreground">{memory.caption}</p>}
                    {tagged.length > 0 && (
                      <div className="flex items-center gap-1.5 mt-1.5">
                        <Users className="h-3 w-3 text-muted-foreground" />
                        <span className="text-xs text-muted-foreground">with {tagged.map(t => t?.display_name).join(', ')}</span>
                      </div>
                    )}
                  </div>
                )}
