...
export function TeammatesSummary() {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {previews.length > 0 && (
          <div className="mt-2 flex -space-x-2">
            {previews.map(t => (
              <div key={t.user_id} className="h-7 w-7 rounded-full overflow-hidden ring-2 ring-white bg-[hsl(var(--brand-navy))]/20 flex items-center justify-center text-xs">
                {t.profile_photo ? (
            ...
            {count > previews.length && (
              <div className="h-7 w-7 rounded-full ring-2 ring-white bg-[hsl(var(--brand-navy))] text-white text-[10px] font-bold flex items-center justify-center">
                +{count - previews.length}
              </div>
            )}
          </div>
      ...
      <ChevronRight className="h-5 w-5 text-[hsl(var(--brand-navy))]/60" />
    </button>
  );
}
