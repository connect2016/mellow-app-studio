...
import { Plus, MapPin, Users, Clock, Play, ChevronRight } from 'lucide-react';
...
function CreateCrawlForm({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
      ...
      <Input
        placeholder="Crawl name (e.g. 'Friday Night Crawl')"
        value={title}
      ...
      <div>
        ...
        <Select value={startBar} onValueChange={setStartBar}>
          <SelectTrigger className="rounded-xl">
            <SelectValue placeholder="Pick the first bar" />
          </SelectTrigger>
          <SelectContent className="max-h-60">
            {WRIGLEYVILLE_BARS.map((bar) => (
              <SelectItem key={bar.id} value={bar.name}>{bar.name}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
        ...
        <div className="flex gap-2 mt-2">
          <Select value={addingStop} onValueChange={setAddingStop}>
            <SelectTrigger className="rounded-xl flex-1">
              <SelectValue placeholder="Add another bar" />
            </SelectTrigger>
            <SelectContent className="max-h-60">
              {WRIGLEYVILLE_BARS.filter(b => b.name !== startBar && !stops.includes(b.name)).map((bar) => (
                <SelectItem key={bar.id} value={bar.name}>{bar.name}</SelectItem>
              ))}
            </SelectContent>
          </Select>
          <Button size="sm" variant="secondary" onClick={handleAddStop} className="rounded-xl">
      ...
      <div className="flex gap-2">
        ...
        <Button
          ...
          disabled={!title || !startBar || !startTime || createCrawl.isPending}
        >
...
export default function PubCrawl() {
  ...
  const { crawls, isLoading, getStopsForCrawl, getMembersForCrawl, isUserInCrawl, joinCrawl, leaveCrawl, updateStatus } = usePubCrawls();
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      <div className="mx-auto max-w-lg px-4 pt-6">
        ...
        {isLoading && (
          <div className="text-center py-12 text-muted-foreground">Loading crawls...</div>
        )}
              ...
              >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 text-xs text-muted-foreground">
                      <span className="flex items-center gap-1">
                        ...
                        {format(new Date(crawl.start_time), 'MMM d · h:mm a')}
                      </span>
                      <span className="flex items-center gap-1">
                ...
                <div className="flex gap-2">
                  {isCreator && crawl.status === 'planning' && (
                    <Button
                      size="sm"
                      className="flex-1 rounded-xl gap-1.5 min-h-[44px] text-xs font-bold"
                  ...
                  {!inCrawl ? (
                    <Button
                      size="sm"
                      className="flex-1 rounded-xl min-h-[44px] text-xs font-bold"
                      onClick={() => joinCrawl.mutate(crawl.id)}
                      disabled={joinCrawl.isPending}
                    >
                      Join Crawl 🍻
                    </Button>
                  ) : (
                    <Button
                      ...
                      variant="outline"
                      className="flex-1 rounded-xl min-h-[44px] text-xs font-bold"
                      onClick={() => leaveCrawl.mutate(crawl.id)}
                      disabled={leaveCrawl.isPending}
                    >
                      Leave
                    </Button>
                  )}
          ...
          {!isLoading && crawls.length === 0 && !showCreate && (
            <div className="text-center py-16 space-y-3">
              ...
              <Button onClick={() => setShowCreate(true)} className="rounded-xl">
                <Plus className="h-4 w-4 mr-1" /> Create a Crawl
              </Button>
            </div>
          )}
        </div>
